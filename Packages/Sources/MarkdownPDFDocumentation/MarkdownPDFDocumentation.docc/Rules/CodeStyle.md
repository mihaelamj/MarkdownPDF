...
The operational namespacing discipline for MarkdownPDF: folder mirroring, file naming, anchor placement, one-type-per-file, reserved names, namespace-vs-module collisions.
...
This applies to every Swift project. The MarkdownPDF-style root + sub-namespace pattern below is one specific instance of this discipline.
...
```text
MarkdownPDF.Parser.Model.Token          // from MarkdownPDFCore
MarkdownPDF.Generator.Command           // from MarkdownPDFCLI
MarkdownPDF.Parser.Scanning.Scanner     // from MarkdownPDFTools
```
...
## Root + sub-namespace pattern (MarkdownPDF style)
...
```swift
/// Root namespace for all MarkdownPDF types across all modules
...
- Root namespace = one file (e.g. `MarkdownPDF.swift`)
...
extension MarkdownPDF.Parser.Model {
...
extension MarkdownPDF.Parser.Model.Token: CustomStringConvertible {
...
- Type lives under the correct semantic path, e.g.:
  - `MarkdownPDF.Model`
  - `MarkdownPDF.Parser.Model`
  - `MarkdownPDF.Parser.Metadata`
  - `MarkdownPDF.Generator`
- Conformances (`Codable`, `Sendable`, `CustomStringConvertible`, etc.) MAY be separate extensions

### File layout for MarkdownPDF-style namespacing
...
```text
MarkdownPDF/
    MarkdownPDF.swift                       // namespace anchor: public enum MarkdownPDF {}
    Parser/Model/Token.swift             // extension MarkdownPDF.Parser.Model { public enum Token {} }
    Parser/Scanning/Scanner.swift        // extension MarkdownPDF.Parser.Scanning { public struct Scanner {} }
    Generator/Writer.swift               // extension MarkdownPDF.Generator { public struct Writer {} }
```
...
```text
MarkdownPDF.swift                              // namespace anchor: public enum MarkdownPDF {}
MarkdownPDF.Parser.Model.Token.swift           // extension MarkdownPDF.Parser.Model { public enum Token {} }
MarkdownPDF.Parser.Metadata.Header.swift       // extension MarkdownPDF.Parser.Metadata { public struct Header {} }
```
...
- `MarkdownPDF.swift` declaring `public enum MarkdownPDF { /* sub-namespaces */ }`
...
Concrete example, a `MarkdownPDF` namespace umbrella:

```text
Sources/MarkdownPDF/
├── MarkdownPDF.swift          // namespace anchor: extension MarkdownPDF { ... }      <-- AT ROOT, not Core/MarkdownPDF.swift
├── Client/                // MarkdownPDFClient SPM target
├── Core/                  // MarkdownPDFCore SPM target (path includes ../MarkdownPDF.swift)
│   ├── Protocol/
│   ├── Server/
│   └── Transport/
├── SharedTools/           // MarkdownPDFSharedTools SPM target
└── Support/               // MarkdownPDFSupport SPM target
```

The MarkdownPDFCore target's `Package.swift` entry uses `path: "Sources/MarkdownPDF"` with `exclude: ["Client", "SharedTools", "Support"]` so it picks up the anchor file plus the `Core/` subtree. Every sibling target (Client, SharedTools, Support) depends on MarkdownPDFCore in production, so the `MarkdownPDF` namespace anchor reaches them through that dep.

Why this matters:

- Reading the file listing immediately tells you "MarkdownPDF is the umbrella here": the anchor sits visually at the top.
- Moving an anchor into `Core/MarkdownPDF.swift` (or any other sub-target folder) hides the umbrella relationship behind one extra `cd` and one extra mental indirection.

...
MarkdownPDF already has more than one target (the `MarkdownPDF` library and the `markdownpdf` CLI). Add a new target to the single package when one of these is true:

- A part of MarkdownPDF has become a clearly separable responsibility (a parser, a transport, a renderer) used in more than one place.
...
For MarkdownPDF specifically, keep the `MarkdownPDF` library name as the public anchor and grow new packages around it (for example `TileCore`, `TileParser`) rather than renaming it.
...
As MarkdownPDF grows targets within its single package, the layout might look like this. Your actual target list will differ.
...
│
├── Domain Layer
│   └── MarkdownPDF           # the main library; depends: TileCore, TileParser
│
└── Front-door (not under Sources/)
    └── markdownpdf         # the CLI executable; wires the concretes
```

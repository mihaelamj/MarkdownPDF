...
How to structure MarkdownPDF's Swift so the same sources build and run on Apple platforms, Linux, and Windows without silent breakage.
...
Grounded in Swift Evolution and `swiftlang/swift-foundation`. The MarkdownPDF engine targets macOS and Linux (a server-style Apple + Linux split), so it falls under Topology 2/3 below. Subprocess and shell-out are available on both macOS and Linux; the engine may use them.
...
3. **Cross-platform library or CLI.** Runs identically on Apple + Linux + Windows. No UI. Probably uses URLSession-via-FoundationNetworking, Darwin/Glibc conditionals, swift-system. The MarkdownPDF engine lives here (macOS + Linux): both platforms support subprocess and shell-out, so the engine may spawn subprocesses when needed.
...
Same pattern for `appleOnlyTargets`. Core engine products (e.g. `MarkdownPDF`) stay unconditional.
...
These patterns (Patterns 1-7 and Pattern 13: iOS-only SwiftUI modifiers, UIKit/AppKit app shells, the forward-compat SDK shim, and the `@available(iOS ...)` examples) describe the planned native Apple UI app, NOT the MarkdownPDF engine. The engine targets macOS + Linux and has no UI. Apply these only in the app tier.
...
```swift
#if os(iOS)
return "MarkdownPDF/iOS/\(version)"
#elseif os(macOS)
return "MarkdownPDF/macOS/\(version)"
#elseif os(visionOS)
return "MarkdownPDF/visionOS/\(version)"
#else
return "MarkdownPDF/unknown/\(version)"  // Linux falls into "unknown"
#endif
```
...
return "MarkdownPDF/visionOS/\(version)"
#elseif os(Linux)
return "MarkdownPDF/Linux/\(version)"
#else
return "MarkdownPDF/unknown/\(version)"  // truly unknown future platform
#endif
```

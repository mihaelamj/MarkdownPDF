...
Canonical dependency-injection principles for MarkdownPDF and the wider MarkdownPDF codebase: no singletons, constructor injection, protocol seams.
...
If and when MarkdownPDF grows into multiple SPM packages:
...
If and when MarkdownPDF is split into multiple packages, each package should lift out of the repo without modification:
...
If MarkdownPDF ships multiple binaries, the **executable target name** stays normal (no `Impl` suffix). The binary's external identity (filename on disk, `which` output) stays clean.

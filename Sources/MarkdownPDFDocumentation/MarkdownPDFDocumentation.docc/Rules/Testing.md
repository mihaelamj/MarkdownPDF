...
How to write tests for MarkdownPDF: focused, isolated, deterministic suites built on the Swift Testing framework.

Write comprehensive tests using the Swift Testing framework (`@Test`). Tests must be focused, isolated, deterministic, and leverage modern Swift Testing features for maximum reliability and maintainability. The core engine (`MarkdownPDF`, the `Tile` / `TileType` primitives, the `MarkdownPDF` namespace) is non-UI and is exercised with plain unit and integration tests. The SwiftUI/ViewInspector/snapshot patterns below apply to the planned native macOS/iOS app, not to the engine.

The `withDependencies` overrides shown in the patterns below assume the Point-Free Dependencies library, whose adoption is an open question; see [../decisions/point-free-dependencies.md](../decisions/point-free-dependencies.md). Where MarkdownPDF does not use that library, control collaborators through plain constructor injection instead; the test-isolation and determinism rules apply either way.
...
let tileKitTarget = Target.target(
    name: "MarkdownPDF",
    dependencies: ["MarkdownPDFModels", "FileClient"]
)
...
MarkdownPDF is a monorepo from day one: sources live under `Packages/Sources/<SourceTarget>/` and tests under `Packages/Tests/<SourceTarget>Tests/`, mirroring each other:

...
How the repository is laid out. MarkdownPDF is a monorepo from day one: a workspace, a single `Package.swift` under `Packages/`, many single-responsibility targets in that one package, and `Apps/` for app targets. Sources live in `Packages/Sources/`, tests in `Packages/Tests/`. The `MarkdownPDF` library and the `markdownpdf` executable are already two targets in that package, so the layout rules below apply now.

## What this covers

MarkdownPDF ships as one SPM package with many targets, not one package per library. There is a single `Package.swift` under `Packages/`; the `MarkdownPDF` library and the `markdownpdf` CLI are two targets in it today, and new targets join the same manifest as responsibilities separate out. The workspace and `Apps/` directory are part of the structure from the start; `Apps/` holds app targets (such as the planned native macOS/iOS editor) while the CLI executable target stays in `Package.swift`.

Reach for additional targets in the single package when one of these is true:

- A part of MarkdownPDF becomes a clearly separable responsibility (a parser, a transport, a renderer) used in more than one place.
...
│   ├── Package.resolved
│   ├── Sources/
│   │   ├── MarkdownPDF/               # the engine library
│   │   ├── MarkdownPDFCLI/           # the markdownpdf CLI target
...
let baseProducts: [Product] = [
    ...
    .executable(name: "markdownpdf", targets: ["MarkdownPDFCLI"]),
]
